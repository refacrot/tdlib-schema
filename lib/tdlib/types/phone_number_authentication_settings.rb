module TD::Types
  ...
  #
  # @attr allow_flash_call [Boolean] Pass true if the authentication code may be sent via a flash call to the
  #   specified phone number.
  # @attr allow_missed_call [Boolean] Pass true if the authentication code may be sent via a missed call to the
  #   specified phone number.
  # @attr is_current_phone_number [Boolean] Pass true if the authenticated phone number is used on the current
  #   device.
  ...
  # @attr authentication_tokens [Array<TD::Types::String>] List of up to 20 authentication tokens, recently received
  #   in updateOption("authentication_token") in previously logged out sessions; for setAuthenticationPhoneNumber only.
